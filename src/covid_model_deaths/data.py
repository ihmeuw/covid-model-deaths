...
from db_queries import get_location_metadata, get_age_metadata, get_population

MEASURES = ['age_death', 'age_pop', 'confirmed', 'deaths', 'full_data', 'us_pops']
...
def get_input_data(measure, data_version='best'):
...
def plot_crude_rates(df, level=None, index_measure='Deaths'):
    # subset if needed
    if level == 'subnat':
...
class DeathModelData:
    def __init__(self, df, age_pop_df, age_death_df, standardize_location_id, model_type, 
                 subnat=False, log_diff=False, rate_threshold=-15):
        # set rate
        ...
        assert len(df[['Location']].drop_duplicates()) == len(df[['location_id']].drop_duplicates()) \
            == len(df[['Country/Region', 'Location']].drop_duplicates()) == len(df[['location_id', 'Country/Region', 'Location']].drop_duplicates()), \
        ...
        # get implied death rate based on "standard" population (using average of all possible locations atm)
        implied_df = age_death_df.loc[age_death_df.location_id == 1, 
                                           ['age_group', 'death_rate']].reset_index(drop=True)
        self.age_pattern_df = age_pop_df.loc[age_pop_df.location_id == standardize_location_id].merge(implied_df)  # attach to age pattern for scaling in asdr function
    ...
    def _backcast_all_locations(self, df):
        # sort dataset
        df = df.copy()
        ...
        # after we expand out days in the moving average bit, need to check we don't do so for days at the beginning with 2 deaths (happens in Other Counties, WA)
        ...
        # get delta
        obs_diff = df['Observed ln(age-standardized death rate)'].values[1:] - df['Observed ln(age-standardized death rate)'].values[:-1]
        ...
        df['first_point'] = df.groupby(['location_id', 'Country/Region', 'Location'], as_index=False).Days.transform('min')
        ...
        df['last_point'] = df.groupby(['location_id', 'Country/Region', 'Location'], as_index=False).Days.transform('max')
        ...
        delta_df = delta_df.groupby(['location_id', 'Country/Region', 'Location'], as_index=False)['Delta ln(asdr)'].mean()
        ...
        bc_df = pd.concat([
            self._backcast_ln_asdr(bc_location_id, 
                                    ...
                                    delta_df.loc[delta_df['location_id'] == bc_location_id, 
                                                 'Delta ln(asdr)'].item())
            for bc_location_id in bc_location_ids
        ])
        df = df.append(bc_df)
        df = df.sort_values(['location_id', 'Days']).reset_index(drop=True)
        df[['Location', 'Country/Region', 'population']] = df[
            ['Location', 'Country/Region', 'population']
    ...
    def _backcast_ln_asdr(self, location_id, df, bc_step):
        # get first point
        start_rep = df.sort_values('Days').reset_index(drop=True)['ln(age-standardized death rate)'][0]
        
        # backcast if above threshold (already dropped below, so other case is == threshold)
        if start_rep > self.rate_threshold:
            ...
            # remove fractional step from last  (we force the threshold day to be 0, so the partial day ends up getting added onto the first day)
            # no longer add date, since we have partial days
            if df['Days'].min() != 0: 
            ...
            bc_df = bc_df.loc[bc_df['Days'] >= -10, ['location_id', 'Days', 'ln(age-standardized death rate)']].reset_index(drop=True)
        else:
    ...
    def _moving_average_lnasdr(self, df):
        ...
        df.loc[df['Date'].isnull(), 'Date'] = df.loc[df['Date'].isnull(), 'Days'].apply(lambda x: df['Date'].min() + timedelta(days=x))
        ...
        # replace last point w/ daily value over 3->2 and 2->1 and the first with 1->2, 2->3; use observed if 3 data points or less
        if len(df) > 3:
            last_step = np.mean(np.array(avgs[-3:-1]) - np.array(avgs[-4:-2]))
            df['ln(age-standardized death rate)'][len(df)-1] = df['ln(age-standardized death rate)'][len(df)-2] + last_step
