...
def plot_death_model(draw_df, location_id, loc_table, log = False, show = True):
    ...
    plt.fill_between(
        ...
        np.percentile(draw_df.loc[draw_df['location_id'] == location_id, [i for i in draw_df.columns if i.startswith('draw_')]], 2.5, axis=1),
        np.percentile(draw_df.loc[draw_df['location_id'] == location_id, [i for i in draw_df.columns if i.startswith('draw_')]], 97.5, axis=1),
        alpha=0.25
    )
    plt.plot(
        draw_df.loc[draw_df['location_id'] == location_id, 'date'],
        draw_df.loc[draw_df['location_id'] == location_id, [i for i in draw_df.columns if i.startswith('draw_')]].mean(axis=1),
    )
    plt.scatter(
...
def save_plots(in_path, out_dir, loc_table, show = True, agg = True):
...
def run_plots(datestamp_label, version):
#     """
#     args = argparse.Namespace(
#         datestamp_label='2020_04_01_Europe'
#         version='1',
#     """
#     parser = argparse.ArgumentParser()
#     parser.add_argument(
#         '--datestamp_label', help='Label identifying input file name', type=str
#     )
#     parser.add_argument(
#         '--version', help='Version for todays plots', type=str
#     )
#     args = parser.parse_args()
#     date = args.datestamp_label[:10]
