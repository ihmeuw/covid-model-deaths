...
class SocialDistCov:
    ...
    def __init__(self, death_df, date_df=None, data_version='best'):
    ...
    def _process_closure_dataset(self):
        ...
        df.loc[df['Location'].isin(list(RENAME.keys())), 'Location'] = df.loc[df['Location'].isin(list(RENAME.keys())), 'Location'].map(RENAME)
        
        ## subnational extracted for France, just keep one and call it national
        #df = df.loc[(df['Country/Region'] != 'France') | (df['admin1'] == 'Corse')]
        #df.loc[df['Country/Region'] == 'France', 'Location'] = 'France'
    ...
    @staticmethod
    def _get_threshold_date(df, date_df):
        # walk back from the first real date to day 0 in the model
        df = df.loc[~df['Date'].isnull()].copy()
        df['first_date'] = df.groupby(['location_id', 'Location', 'Country/Region'], as_index=False)['Date'].transform('min')
        ...
        if date_df is not None:
            df = df.append(
                date_df.loc[~date_df['Location'].isin(df['Location'].unique().tolist()), ['Location', 'Country/Region', 'threshold_date']]
            ).reset_index(drop=True)
        
        return df
    
    def _calc_composite(self, weights):
        # scale weights
        ...
        return df[['Location', 'Country/Region', 'threshold_date'] + closure_vars + ['composite_1w', 'composite_2w', 'composite_3w']]
        
    def get_cov_df(self, weights=[1, 1, 1], k=20):
        # get composites
