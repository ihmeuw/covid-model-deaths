...
def get_previous_filepaths(in_dir: str, location: str, n_models=3):
    """`location` should be `US` or `Europe` for now."""
    if location not in ['US', 'Europe']:
        raise ValueError(f"{location} is not US or Europe")

    lst_paths = []
    for n_diff in range(0, 100):
        prev_date = get_previous_date(n_diff)
        prev_dir = f"{prev_date}_{location}"
        prev_filepath = get_filepath(in_dir, prev_dir, location)
...
def average_model_outputs(lst_paths, model_1, indir, location):
    ...
    mean_df = (mean_df.groupby(['location_id', 'location'])[DRAWS].cumsum().join(tmp_cols)[COLUMNS + DRAWS])
    return mean_df


def get_today_data(in_dir, location):
    today_date = datetime.today().strftime("%Y_%m_%d").replace('-', '_')
    today_dir = f"{today_date}_{location}"
    today_filepath = get_filepath(in_dir, today_dir, location)
...
def moving_average_predictions(location, in_dir="/ihme/covid-19/deaths/prod",
    ...
    mean_df = average_model_outputs(lst_paths, model_1, in_dir, location)
