...
class Drawer:
    ...
    def _collect_draws(self, ensemble_dir):
        ...
        with open(f'{ensemble_dir}/{self.location_name}/tight_models.pkl', 'rb') as fread:
            tight_models = pickle.load(fread)
        ...
        if self.location_tag in list(model_draws.keys()) and \
            len(tight_models[self.location_tag].obs) >= 5 and \
        ...
        return model_used, days, draws
    
    def _get_dated_df(self, days, draws):
        # apply dates
        df = pd.concat([
                pd.DataFrame({
                    'date':date,
    ...
    def get_dated_draws(self):
        ...
        return draw_df, model_used, days, ensemble_draws
